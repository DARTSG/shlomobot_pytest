...
def function_contains_input(function: FunctionType, monkeypatch=None) -> bool:
    """
    Checks if a given function contains a call to the builtin
    `input` function

    This function is overloaded with 2 implementations. If `monkeypatch`
    is given, it uses `monkeypatch` to check if the function is called. This
    is more accurate and captures only when the function is actually called.

    If `monkeypatch` is not given, it uses a regular expression to identify
    calls. This is less accurate and does not account for occurrences like
    the following, on which it would incorrectly return True:
    ...
    if monkeypatch:
        FUNCTION_NAME = "builtins.input"
        function_call_status = function_calls_other_function(monkeypatch, function, FUNCTION_NAME)
        return function_call_status[FUNCTION_NAME]
    else:
