...
def module_contains_mandatory_imports(
    module_name: str, imports_required: list[str]
) -> bool:
    imports_made = get_imported_modules(module_name)
    return all(
        import_statement in imports_made for import_statement in imports_required
    )
